...
- **First pocket pass** ✅ 2026-06-11 (`..._pocket_normal_20260611-133605.jsonl`; `*_anchors-fixed` variant has checkpoint times recovered from 3-s standing pauses — the surveyor can't tap with the phone pocketed, so taps were an end-burst; future surveys: pause ≥3 s at each checkpoint). Result: **magnetic matching survives pocket** (research predicted 3–6 m degradation): 6/6 fires in order, delays −1.7…+10.8 s. Two pocket defects: (a) leg-swing distorts gyro turn magnitudes → false unmatched U-turn → **false off-route flag on a clean walk** (turn params are hand-pose-fitted; need pose-aware thresholds or down-weighted turn evidence in pocket), (b) noisier posterior cascades the final checkpoint −5.5 s early. RoNIN (Phase 4) not yet forced — collect 1–2 more pocket passes before deciding.
